...
export function * searchLocations (api, action) {
  const { query } = action
  // make the call to the api
  const apiCall = call(api.searchLocations, query)
  const response = yield call(callApi, apiCall)

  // success?
  if (response.ok) {
    yield put(LocationActions.locationSearchSuccess(response.data))
  } else {
    yield put(LocationActions.locationSearchFailure(response.data))
  }
}
