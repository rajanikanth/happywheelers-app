...
import { getLocation, getLocations, updateLocation, deleteLocation, searchLocations } from '../../../../../app/modules/entities/location/location.sagas'
...
test('search success path', () => {
  const response = FixtureAPI.searchLocations()
  const step = stepper(searchLocations(FixtureAPI, '*'))
  // Step 1: Hit the api
  step()
  // Step 2: Successful return and data!
  expect(step(response)).toEqual(put(LocationActions.locationSearchSuccess([{ id: 1 }, { id: 2 }])))
})
...
test('search failure path', () => {
  ...
  step()
  // Step 2: Failed response.
  expect(step(response)).toEqual(put(LocationActions.locationSearchFailure()))
})
