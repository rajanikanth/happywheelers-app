...
test('attempt searching a location', () => {
  const state = reducer(INITIAL_STATE, Actions.locationSearchRequest(1))
...
test('success searching a location', () => {
  const state = reducer(INITIAL_STATE, Actions.locationSearchSuccess({ id: 1 }))

  expect(state.searching).toBe(false)
  expect(state.errorSearching).toBe(null)
  expect(state.locations).toEqual({ id: 1 })
})
...
test('failure searching a location', () => {
  const state = reducer(INITIAL_STATE, Actions.locationSearchFailure({ error: 'Not found' }))
